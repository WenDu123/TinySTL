#pragma once
#ifndef __MULTI_MAP_H_
#define __MULTI_MAP_H_
   ...
   //不允许通过迭代器改变键值，允许改变实值
	...
	public:
		...
		class value_compare
			...
			friend multimap<Key, T, Compare, Alloc>;
		protected:
			Compare comp;
	...
	public:
		...
		multimap(const multimap& m) : t(m.t) {}
		...
		3. ((insert(value_type(k, T()))).first) 取出iterator   无需此步
